...
@section('content')
    <div class="page-header-modern">
        <h1><i class="bi bi-pencil-fill me-2" style="color: var(--primary-color);"></i>Edit Lead</h1>
    ...
    <div class="card card-modern">
        <div class="card-body">
            <form action="{{ route('leads.update', $lead) }}" method="POST" class="form-modern">
                @csrf @method('PUT')
                <div class="row g-4">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('event_type') is-invalid @enderror" id="event_type"
                            ...
                            @foreach(['Wedding', 'Corporate', 'Birthday', 'Portrait', 'Product', 'Other'] as $type)
                                <option value="{{ $type }}" {{ old('event_type', $lead->event_type) == $type ? 'selected' : '' }}>
                                    {{ $type }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control @error('event_date') is-invalid @enderror" id="event_date"
                            name="event_date" value="{{ old('event_date', $lead->event_date?->format('Y-m-d')) }}">
