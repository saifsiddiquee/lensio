...
@section('content')
    <div class="page-header-modern">
        <h1><i class="bi bi-file-earmark-check-fill me-2" style="color: var(--success-color);"></i>Create New Contract</h1>
        <a href="{{ route('contracts.index') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>
            Back</a>
    </div>
    <div class="card card-modern">
        <div class="card-body">
            <form action="{{ route('contracts.store') }}" method="POST" class="form-modern">
                @csrf
                <div class="row g-4">
                    <div class="col-md-6"><label for="event_id" class="form-label">Event *</label><select
                            ...
                            <option value="">Select Event</option>@foreach($events as $event)<option
                                ...
                                {{ $event->event_type }} - {{ $event->client->name }}
                            ({{ $event->event_date->format('M d') }})</option>@endforeach
                    ...
                    <div class="col-md-6"><label for="reference_no" class="form-label">Reference No *</label><input
                            ...
                            name="reference_no" value="{{ old('reference_no') }}" required>@error('reference_no')<div
                ...
                <div class="mt-4 pt-3 border-top"><button type="submit" class="btn btn-modern btn-modern-success"><i
                            class="bi bi-check-lg me-1"></i> Create Contract</button><a
