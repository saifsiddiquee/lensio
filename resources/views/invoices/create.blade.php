...
@section('content')
    <div class="page-header-modern">
        <h1><i class="bi bi-receipt me-2" style="color: var(--warning-color);"></i>Create New Invoice</h1>
        <a href="{{ route('invoices.index') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>
            Back</a>
    </div>
    <div class="card card-modern">
        <div class="card-body">
            <form action="{{ route('invoices.store') }}" method="POST" class="form-modern">
                @csrf
                <div class="row g-4">
                    ...
                    <div class="col-md-6"><label for="invoice_no" class="form-label">Invoice No *</label><input type="text"
                            ...
                            value="{{ old('invoice_no', 'INV-' . date('Ymd') . '-' . rand(100, 999)) }}"
                            required>@error('invoice_no')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                    <div class="col-md-6"><label for="total_amount" class="form-label">Total Amount *</label>
                        <div class="input-group"><span class="input-group-text">৳</span><input type="number" step="0.01"
                                ...
                                name="total_amount" value="{{ old('total_amount') }}" required></div>@error('total_amount')
                    ...
                    <div class="col-md-6"><label for="status" class="form-label">Status *</label><select
                            ...
                            required>@foreach($statuses as $status)<option value="{{ $status }}" {{ old('status', 'unpaid') == $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
                ...
                <div class="mt-4 pt-3 border-top"><button type="submit" class="btn btn-modern btn-modern-primary"><i
                            class="bi bi-check-lg me-1"></i> Create Invoice</button><a href="{{ route('invoices.index') }}"
