...
@section('content')
    <div class="page-header-modern">
        <h1><i class="bi bi-people-fill me-2" style="color: var(--success-color);"></i>Create New Client</h1>
    ...
    <div class="card card-modern">
        <div class="card-body">
            <form action="{{ route('clients.store') }}" method="POST" class="form-modern">
                @csrf
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('lead_id') is-invalid @enderror" id="lead_id" name="lead_id"
                            ...
                            @foreach($leads as $lead)<option value="{{ $lead->id }}" {{ old('lead_id') == $lead->id ? 'selected' : '' }}>{{ $lead->name }} - {{ $lead->event_type }}</option>@endforeach
                        </select>
